...
# Animation states
enum AnimState { IDLE, MOVING, FIRING, SUPPRESSED, DEAD }
...
func setup(target: Node3D, model: Node3D, is_vehicle: bool = false, is_helicopter: bool = false) -> void:
	_target_node = target
	_model_node = model
	...
	if model:
		...
		# Try to find turret node for vehicles
		if is_vehicle:
...
func _animate_vehicle(delta: float) -> void:
	...
	# Animate turret rotation toward target
	if _turret_node:
