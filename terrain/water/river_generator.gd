...
## Returns Array of RiverPath objects (typed array causes parser issues with inner class)
## Note: heightmap untyped to avoid parser resolution issues with HeightmapStorage
func extract_rivers_fast(heightmap, river_count: int = 8) -> Array:
	var paths: Array = []
	var map_size: int = heightmap.size
	var _cell_size: float = heightmap.cell_size  # Reserved for world-space conversion
...
func _trace_river_downhill(heightmap, start: Vector2i, visited: Dictionary) -> RiverPath:
...
func extract_rivers(heightmap, threshold: float = 0.3) -> Array:
	var map_size: int = heightmap.size
	...
	var paths: Array = _trace_river_paths(river_cells, accumulation, flow_dir, heightmap)
...
func _compute_flow_directions(heightmap) -> PackedByteArray:
	var map_size: int = heightmap.size
...
func paths_to_cells(paths: Array, cell_size: float) -> Array[PackedVector2Array]:
