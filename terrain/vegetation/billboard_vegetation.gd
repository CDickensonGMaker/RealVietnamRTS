...
# Billboard materials (reserved for future texture-based billboards)
...
func _create_billboard_meshes() -> void:
	# Create tree billboard meshes (reduced height for better RTS visibility)
	for tex in _tree_textures:
		var mesh := _create_cross_billboard_mesh(tex, 5.0, 7.0)  # Shorter trees (was 6.0, 10.0)
		_tree_meshes.append(mesh)

	# Create bamboo billboard meshes (tall, narrow - reduced)
	for tex in _bamboo_textures:
		var mesh := _create_cross_billboard_mesh(tex, 3.0, 8.0)  # Shorter bamboo (was 4.0, 12.0)
		_tree_meshes.append(mesh)  # Add to tree meshes for variety

	# Create bush mesh (short, wide)
	if not _bush_textures.is_empty():
		_bush_mesh = _create_cross_billboard_mesh(_bush_textures[0], 2.5, 2.0)  # Slightly smaller

	# Create rice billboard meshes (short grass clumps for paddies)
	for tex in _rice_textures:
		var mesh := _create_cross_billboard_mesh(tex, 1.5, 1.0)  # Short and wide
