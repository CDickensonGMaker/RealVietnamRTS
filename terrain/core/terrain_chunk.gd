...
static func _create_shared_material() -> void:
	...
	if ResourceLoader.exists(shader_path):
		...
		if shader:
			...
			var default_clearing := Image.create(1, 1, false, Image.FORMAT_RGBA8)
			default_clearing.fill(Color(0, 0, 0, 0))  # Fully transparent
			var clearing_tex := ImageTexture.create_from_image(default_clearing)
			shader_mat.set_shader_parameter("clearing_texture", clearing_tex)
