...
## Load chunks around camera initially (not all chunks)
func _load_initial_chunks() -> void:
	# Only load chunks around camera, not all chunks
	# The streaming system will load more as camera moves
	var center_chunk: Vector2i
	if camera:
		center_chunk = _world_to_chunk(camera.global_position)
	else:
		# Fallback to center of map
		center_chunk = Vector2i(chunks_per_side / 2, chunks_per_side / 2)

	_load_chunks_around(center_chunk, load_distance)
